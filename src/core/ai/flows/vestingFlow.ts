...
export function vestingFlow(
  ...
  if (state.step === 0) {
    return startVestedGuidedFlow();
  }

  if (state.step === 1 && isVestedGuidedContext(ctx)) {
    return runVestedGuidedFlow(state, input.trim(), structured);
  }
