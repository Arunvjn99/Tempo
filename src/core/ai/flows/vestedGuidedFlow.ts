...
import { VESTED_AI_STATE_KEY, type VestedAIState } from "../state/vestedAIState";
...
function vestedNextState(vestedAI: VestedAIState): LocalFlowState {
  return {
    type: "vesting",
    step: 1,
    context: { guided: true, [VESTED_AI_STATE_KEY]: vestedAI },
  };
}
...
export function startVestedGuidedFlow(): LocalAIResult {
  ...
  return {
    messages: [
      assistantMessage("Your balance.", {
        interactiveType: "balance_card",
        interactivePayload: balancePayload(),
...
export function runVestedGuidedFlow(
      ...
      return {
        messages: [
          assistantMessage("Insight.", {
            interactiveType: "info_card",
            interactivePayload: insightPayload(),
          }),
        ],
        nextState: vestedNextState(next),
      };
    }
    return {
      messages: [
        assistantMessage("Say **continue** for insight.", {
          interactiveType: "balance_card",
          interactivePayload: balancePayload(),
          suggestions: ["Continue"],
        }),
  ...
  if (vestedAI.step === "insight") {
    return {
      messages: [
        assistantMessage("Tap **Done** to return.", {
          interactiveType: "info_card",
          interactivePayload: insightPayload(),
        }),
      ],
