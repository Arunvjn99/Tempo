...
import { ENROLLMENT_AI_STATE_KEY, type EnrollmentAIState } from "../state/enrollmentAIState";
...
function enrollmentNextState(enrollmentAI: EnrollmentAIState): LocalFlowState {
  return {
    type: "enrollment",
    step: 1,
    context: { guided: true, [ENROLLMENT_AI_STATE_KEY]: enrollmentAI },
  };
}
...
function setupPayload(): EnrollmentSetupPayload {
  return {
    planOptions: [
      { label: "Pre-tax (Traditional 401k)", value: "traditional" },
      { label: "Roth 401k", value: "roth" },
    ],
    contributionMin: 1,
    contributionMax: 20,
    contributionValue: 5,
...
function enrollmentReviewPayload(enrollmentAI: EnrollmentAIState): EnrollmentReviewPayload {
  return {
    plan: enrollmentAI.data.plan ?? "roth",
    contribution: enrollmentAI.data.contribution ?? 5,
    investment: enrollmentAI.data.investment ?? "target",
  };
}
...
function buildSuccessPayload(): SuccessCardPayload {
  return {
    ...
    description: "Your retirement plan is now active",
    processingTime: "Active immediately",
    reassuranceMessage: "Your first contribution will be taken from your next paycheck.",
    actionLabel: "Done",
  };
}
...
export function startEnrollmentGuidedFlow(): LocalAIResult {
  ...
  return {
    messages: [
      assistantMessage("Set up your plan in one step.", {
        interactiveType: "enrollment_setup_card",
        interactivePayload: setupPayload(),
      }),
    ],
...
export function runEnrollmentGuidedFlow(
  ...
  if (structured) {
    if (enrollmentAI.step === "setup" && structured.action === "enrollment_setup_continue") {
      ...
      return {
        messages: [
          assistantMessage("Review and submit.", {
            interactiveType: "enrollment_review_card",
            interactivePayload: enrollmentReviewPayload(next),
          }),
        ],
    ...
    if (enrollmentAI.step === "review" && structured.action === "enrollment_review_submit") {
      ...
      return {
        messages: [
          assistantMessage("Congratulations!", {
    ...
    if (enrollmentAI.step === "success" && structured.action === "success_card_dismiss") {
      return { messages: [], nextState: null, navigate: "/enrollment" };
    }
  }

  const trimmed = input.trim();
  if (/^(yes|continue|ok)\b/i.test(trimmed)) {
    if (enrollmentAI.step === "setup") {
      return {
        messages: [
          assistantMessage("Use the card above to set plan, contribution, and investment.", {
            interactiveType: "enrollment_setup_card",
            interactivePayload: setupPayload(),
          }),
        ],
    ...
    if (enrollmentAI.step === "review") {
      return {
        messages: [
          assistantMessage("Tap **Submit enrollment** on the card.", {
            interactiveType: "enrollment_review_card",
            interactivePayload: enrollmentReviewPayload(enrollmentAI),
          }),
        ],
        nextState: enrollmentNextState(enrollmentAI),
      };
  ...
  if (enrollmentAI.step === "success") {
    return {
      messages: [
        assistantMessage("Tap **Done** to continue.", {
          interactiveType: "success_card",
          interactivePayload: buildSuccessPayload(),
        }),
      ],
  ...
  return {
    messages: [
      assistantMessage("Use the controls above to continue.", {
        interactiveType: enrollmentAI.step === "setup" ? "enrollment_setup_card" : "enrollment_review_card",
        interactivePayload: enrollmentAI.step === "setup" ? setupPayload() : enrollmentReviewPayload(enrollmentAI),
      }),
    ],
    nextState: enrollmentNextState(enrollmentAI),
  };
