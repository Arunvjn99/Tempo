...
import { WITHDRAWAL_AI_STATE_KEY, type WithdrawalAIState } from "../state/withdrawalAIState";
...
function withdrawalNextState(withdrawalAI: WithdrawalAIState): LocalFlowState {
  return {
    type: "withdrawal",
    step: 1,
    context: { guided: true, [WITHDRAWAL_AI_STATE_KEY]: withdrawalAI },
  };
}
...
function methodPayload(): SelectionCardPayload {
  return {
    ...
    insight: "Bank transfer is usually fastest. Check takes 5–7 business days.",
  };
}
...
function buildSuccessPayload(): SuccessCardPayload {
  return {
    ...
    description: "Funds will be credited in 2–3 business days.",
    processingTime: "2–3 business days",
    reassuranceMessage: "You'll receive a confirmation when the transfer is complete.",
    actionLabel: "Done",
  };
}
...
export function startWithdrawalGuidedFlow(): LocalAIResult {
  ...
  return {
    messages: [
      assistantMessage(`Max: ${money(maxWithdraw)}`, {
        interactiveType: "withdrawal_slider_card",
        interactivePayload: withdrawalSliderPayload(maxWithdraw, 2000),
      }),
    ],
...
export function runWithdrawalGuidedFlow(
  ...
  if (structured) {
    if (withdrawalAI.step === "amount_selection" && structured.action === "withdrawal_amount_continue") {
      ...
      return {
        messages: [
          assistantMessage("Choose disbursement method.", {
    ...
    if (withdrawalAI.step === "method" && structured.action === "selection_card_pick") {
      ...
      return {
        messages: [
          assistantMessage("Review withdrawal.", {
  ...
  if (withdrawalAI.step === "amount_selection") {
    return {
      messages: [
        assistantMessage("Use the slider, then **Continue**.", {
          interactiveType: "withdrawal_slider_card",
          interactivePayload: withdrawalSliderPayload(maxWithdraw, amt),
        }),
      ],
      nextState: withdrawalNextState(withdrawalAI),
    };
  ...
  if (withdrawalAI.step === "method") {
    return {
      messages: [
        assistantMessage("Pick an option.", {
          interactiveType: "selection_card",
          interactivePayload: methodPayload(),
        }),
      ],
  ...
  if (withdrawalAI.step === "review") {
    return {
      messages: [
        assistantMessage("Tap **Submit withdrawal**.", {
          interactiveType: "withdrawal_review_card",
          interactivePayload: withdrawalReviewPayload(withdrawalAI),
        }),
      ],
      nextState: withdrawalNextState(withdrawalAI),
    };
  ...
  if (withdrawalAI.step === "success") {
    return {
      messages: [
        assistantMessage("Tap **Done**.", {
          interactiveType: "success_card",
          interactivePayload: buildSuccessPayload(),
        }),
      ],
  ...
  return { messages: [assistantMessage("Use the controls above.")], nextState: withdrawalNextState(withdrawalAI) };
}
