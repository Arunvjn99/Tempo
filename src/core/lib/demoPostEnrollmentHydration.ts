import type { Scenario, ScenarioActivity, ScenarioPortfolioHolding } from "@/core/engine/scenarioEngine";
import { scenarioRuntimePayload } from "@/core/engine/scenarioEngine";
...
} from "@/core/globalStores/postEnrollmentDashboardStore";
...
function scenarioTxToActivity(t: ScenarioActivity): ActivityEntry {
...
export function applyScenarioToPostEnrollmentDashboard(scenario: Scenario | null): void {
  ...
  const vestedPercent = base.overview.vestedPercent;
  const vestedBalance = Math.round((d.balance * vestedPercent) / 100);
