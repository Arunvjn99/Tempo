...
function EnrollmentLayoutInner() {
  ...
  const sub = location.pathname.replace(/^\/enrollment\/?/, "") || "";
  ...
  return (
    <FigmaScope className="enrollment-figma-flow flex min-h-0 min-h-full flex-1 flex-col bg-[var(--surface-page)]">
      ...
      {showStepper && (
        <div className="border-b border-[var(--border-default)] bg-[var(--surface-card)] px-4 py-3 shadow-[var(--shadow-header)]">
      ...
      {/* Main Content */}
      <main className="flex-1 w-full max-w-5xl mx-auto px-[var(--spacing-6)] py-[var(--spacing-8)]">
        <Outlet />
      </main>
    </FigmaScope>
  );
