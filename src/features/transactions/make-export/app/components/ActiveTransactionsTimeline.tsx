  ...
  const getStatusBadgeClass = (status: Transaction["status"]) => {
    switch (status) {
      case "Action Required":
        return "bg-[color-mix(in_srgb,var(--color-warning)_14%,var(--surface-card))] text-[color-mix(in_srgb,var(--color-warning)_55%,var(--text-primary))] hover:bg-[color-mix(in_srgb,var(--color-warning)_14%,var(--surface-card))] border-[color-mix(in_srgb,var(--color-warning)_28%,var(--border-default))]";
      case "Processing":
    ...
    <Card className="p-6">
      ...
      <div className="space-y-6">
        {mockTransactions.map((transaction) => (
          <div
            key={transaction.id}
            className="p-4 border border-default rounded-lg hover:border-default transition-colors"
          >
            {/* Transaction Header */}
                      ...
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center mb-2 z-10 ${
                          stepStatus === "completed"
                            ...
                            : stepStatus === "current"
                            ? "bg-[var(--color-primary)] text-primary-foreground ring-4 ring-[color-mix(in_srgb,var(--color-primary)_18%,var(--surface-card))]"
