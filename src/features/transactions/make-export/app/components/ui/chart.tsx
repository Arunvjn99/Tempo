...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "border-default/50 bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl",
        className,
      )}
    >
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={item.dataKey}
              className={cn(
                "[&>svg]:text-secondary flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5",
      ...
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn(
              "[&>svg]:text-secondary flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3",
            )}
          >
