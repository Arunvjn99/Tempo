    ...
    <div className="space-y-6">
        ...
        {types.map((type, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                {isSelected && (
                  <CheckCircle2 className="absolute top-5 right-5" style={{ width: 20, height: 20, color: "var(--color-primary)" }} />
                )}
                ...
                <div className="flex items-start gap-4">
                  <div
                    ...
                    style={{
                      ...
                      background: isSelected ? "linear-gradient(135deg, color-mix(in srgb, var(--color-primary) 22%, var(--surface-card)), color-mix(in srgb, var(--color-primary) 28%, var(--surface-card)))" : "var(--surface-muted)",
                  ...
                  <div className="flex-1">
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.3px", marginBottom: 4 }}>
                      {type.title}
                    </h3>
                    <p style={{ fontSize: 13, fontWeight: 500, color: "var(--text-secondary)", marginBottom: 12, lineHeight: "20px" }}>
                      {type.description}
                    </p>
                      ...
                      {type.details.map((detail, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <span
                            ...
                            style={{ width: 6, height: 6, background: isSelected ? "var(--color-primary)" : "var(--border-default)" }}
                          />
                          <span style={{ fontSize: 12, fontWeight: 500, color: "var(--text-secondary)" }}>{detail}</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      >
        <div
          ...
          style={{ background: "linear-gradient(135deg, var(--surface-soft) 0%, color-mix(in srgb, var(--color-primary) 22%, var(--surface-card)) 100%)", border: "1px solid color-mix(in srgb, var(--color-primary) 28%, var(--surface-card))", borderRadius: 14, padding: "14px 16px" }}
        >
