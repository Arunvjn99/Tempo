...
export function WithdrawalEligibility() {
  ...
  const metrics = [
    { icon: <DollarSign className="w-5 h-5" />, label: "Available to Withdraw", value: "$5,000", bg: "var(--surface-soft)", color: "var(--color-primary)" },
    { icon: <Percent className="w-5 h-5" />, label: "Estimated Tax Withholding", value: "20–35%", bg: "color-mix(in srgb, var(--color-warning) 12%, var(--surface-card))", color: "var(--color-warning)" },
    { icon: <Shield className="w-5 h-5" />, label: "Vested Balance", value: "$25,000", bg: "color-mix(in srgb, var(--color-accent) 10%, var(--surface-card))", color: "var(--color-accent)" },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {metrics.map((m, i) => (
            <div key={i} className="flex items-start gap-4">
              ...
              <div>
                <p style={{ fontSize: 12, fontWeight: 500, color: "var(--text-secondary)", marginBottom: 4 }}>{m.label}</p>
                <p style={{ fontSize: 24, fontWeight: 800, color: "var(--text-[var(--color-primary)])", letterSpacing: "-0.5px" }}>{m.value}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {eligibilityChecks.map((check, idx) => (
            <motion.div
              ...
              style={{
                ...
                border: check.eligible ? "1px solid color-mix(in srgb, var(--color-success) 32%, var(--border-default))" : "1px solid var(--border-default)",
                background: check.eligible ? "color-mix(in srgb, var(--color-success) 8%, var(--surface-card))" : "var(--surface-muted)",
              }}
            >
              <div className="flex items-center gap-3">
                {check.eligible ? (
                  <CheckCircle2 className="flex-shrink-0" style={{ width: 18, height: 18, color: "var(--color-success)" }} />
                ) : (
                  <XCircle className="flex-shrink-0" style={{ width: 18, height: 18, color: "var(--border-default)" }} />
                )}
                <div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: check.eligible ? "var(--text-[var(--color-primary)])" : "var(--text-secondary)" }}>
                    {check.label}
                  </p>
                  <p style={{ fontSize: 11, fontWeight: 500, color: "var(--text-secondary)" }}>{check.note}</p>
                </div>
              </div>
              <span style={{
                fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 6,
                background: check.eligible ? "color-mix(in srgb, var(--color-success) 14%, var(--surface-card))" : "var(--surface-muted)",
                color: check.eligible ? "var(--color-success)" : "var(--text-secondary)",
              }}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut", delay: 0.15 }}>
        <FlowInfoBanner variant="warning">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-[var(--color-primary)])", letterSpacing: "-0.3px", marginBottom: 8 }}>
