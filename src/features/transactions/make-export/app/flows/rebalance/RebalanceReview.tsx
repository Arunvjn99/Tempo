    ...
    return (
      ...
      >
        <motion.div
          ...
          className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[color-mix(in_srgb,var(--color-success)_55%,var(--surface-card))] to-[var(--color-success)] flex items-center justify-center mb-6"
              ...
              <div key={step} className="flex items-center gap-3">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                    idx === 0
                      ? "bg-gradient-to-br from-[var(--color-success)] to-[var(--color-success)] text-primary-foreground"
                ...
                <span
                  className={`text-sm ${idx === 0 ? "text-primary font-medium" : "text-secondary"}`}
                >
                  {step}
                </span>
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="font-semibold text-primary">Allocation Changes</h3>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <span className="text-sm text-primary">{fund.name}</span>
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span
                          className={`text-[10px] font-semibold ${change !== 0 ? "text-[var(--color-primary)]" : "text-secondary"}`}
                        >
                  ...
                  <div className="text-right">
                    {change !== 0 ? (
                      <span
                        className={`text-xs font-semibold ${change > 0 ? "text-[var(--color-success)]" : "text-[var(--color-danger)]"}`}
                      >
                        {change > 0 ? "+" : "-"}${tradeAmount.toLocaleString()}
                      </span>
                    ) : (
                      <span className="text-xs text-secondary">No change</span>
                    )}
                  </div>
                </div>
              );
      ...
      <div className="flex justify-between items-center" style={{ paddingTop: 16 }}>
        <button
          onClick={() => navigate("/transactions/rebalance/trades")}
