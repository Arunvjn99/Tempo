...
export function RolloverDocuments() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div
          className="p-5 rounded-2xl border-2 border-dashed border-[color-mix(in_srgb,var(--color-primary)_28%,var(--border-default))] bg-gradient-to-br from-[color-mix(in_srgb,var(--color-accent)_10%,var(--surface-card))]/50 to-[color-mix(in_srgb,var(--color-primary)_10%,var(--surface-card))]/30 hover:border-[color-mix(in_srgb,var(--color-primary)_32%,var(--border-default))] transition-all cursor-pointer group"
          onClick={() => {
        ...
        >
          <div className="flex flex-col items-center text-center py-4">
            <div className="p-3 rounded-xl bg-[color-mix(in_srgb,var(--color-primary)_14%,var(--surface-card))]/70 text-[var(--color-primary)] mb-3 group-hover:bg-[color-mix(in_srgb,var(--color-primary)_14%,var(--surface-card))] transition-colors">
            ...
            >
              <Card
                className={`p-5 rounded-2xl transition-all duration-300 ${
                  isUploaded
                    ? "border-[color-mix(in_srgb,var(--color-success)_28%,var(--border-default))]/60 bg-[color-mix(in_srgb,var(--color-success)_10%,var(--surface-card))]/20"
                    : "border-default/80"
                }`}
                style={{
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {doc.required ? (
                          <span className="text-[9px] font-semibold bg-[color-mix(in_srgb,var(--color-danger)_8%,var(--surface-card))] text-[var(--color-danger)] px-2 py-0.5 rounded-full border border-[color-mix(in_srgb,var(--color-danger)_22%,var(--border-default))]/60">
                            Required
                          </span>
                        ) : (
                          <span className="text-[9px] font-semibold bg-surface-card text-secondary px-2 py-0.5 rounded-full border border-default">
                            Optional
                          </span>
                        )}
                      </div>
                  ...
                  {isUploaded ? (
                    <CheckCircle2 className="w-5 h-5 text-[var(--color-success)] flex-shrink-0 mt-1" />
                  ) : (
                    <Button
                      variant="outline"
                      size="sm"
                ...
                {uploadedForDoc.map((file) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-3 mt-3 bg-background rounded-xl border border-default"
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 rounded-lg bg-surface-card border border-default">
      ...
      <div className="flex justify-between items-center" style={{ paddingTop: 16 }}>
        <button
          onClick={() => navigate("/transactions/rollover/allocation")}
        ...
        <button
          onClick={() => navigate("/transactions/rollover/review")}
