...
export function RolloverReview() {
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[color-mix(in_srgb,var(--color-success)_65%,var(--surface-card))] to-[var(--color-success)] flex items-center justify-center mb-6"
        ...
        <p className="text-secondary text-center max-w-md mb-3">
              ...
              <div key={idx} className="flex items-center gap-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                    step.active
                      ? "bg-gradient-to-br from-[var(--color-accent)] to-[var(--color-accent)] text-primary-foreground"
                      : "bg-surface-card text-secondary border border-default"
                  }`}
                  style={
                    step.active
                      ? {
                ...
                <div>
                  <p
                    className={`text-sm font-medium ${step.active ? "text-primary" : "text-secondary"}`}
                  >
                    {step.label}
                  </p>
                  <p className="text-[10px] text-secondary">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="space-y-6">
        ...
        >
          ...
          <div className="space-y-3">
            {allocation.map((fund, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-3 rounded-xl card-standard"
              >
      ...
      >
        <Card
          className="p-6 rounded-2xl border-default/80 bg-gradient-to-br from-[color-mix(in_srgb,var(--color-accent)_10%,var(--surface-card))]/30 to-[color-mix(in_srgb,var(--color-primary)_10%,var(--surface-card))]/20"
          style={{
            boxShadow:
              "0 1px 2px rgba(0,0,0,0.03), 0 8px 24px rgba(0,0,0,0.05)",
          }}
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={idx} className="text-center">
                <div className="w-8 h-8 rounded-full bg-background border-2 border-[color-mix(in_srgb,var(--color-primary)_28%,var(--border-default))] flex items-center justify-center mx-auto mb-2 text-xs font-semibold text-[var(--color-primary)]">
                  {idx + 1}
                </div>
      ...
      <div className="flex justify-between items-center" style={{ paddingTop: 16 }}>
        <button
          onClick={() => navigate("/transactions/rollover/documents")}
