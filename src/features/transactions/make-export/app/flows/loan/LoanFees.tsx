    ...
    <div className="space-y-4">
      ...
      >
        <div className="card-standard" style={{ padding: "24px 28px" }}>
          ...
          <div className="space-y-0">
            {/* Gross Loan Amount */}
            <div className="flex items-center justify-between" style={{ padding: "14px 0", borderBottom: "1px solid var(--border-default)" }}>
              <div>
                <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)" }}>Gross Loan Amount</p>
                <p style={{ fontSize: 12, fontWeight: 500, color: "var(--text-secondary)" }}>Total amount requested</p>
              </div>
            ...
            {/* Individual Fees */}
            {fees.map((fee, i) => (
              <div key={i} className="flex items-center justify-between" style={{ padding: "12px 0", borderBottom: "1px solid var(--border-default)" }}>
                <div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{fee.label}</p>
                  <p style={{ fontSize: 11, fontWeight: 500, color: "var(--text-secondary)" }}>{fee.desc}</p>
                </div>
                <p style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>-${fee.amount}</p>
              </div>
            ))}

            {/* Total Fees */}
            <div className="flex items-center justify-between" style={{ padding: "14px 0", borderBottom: "1px solid var(--border-default)" }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)" }}>Total Fees</p>
              <p style={{ fontSize: 14, fontWeight: 700, color: "var(--color-danger)" }}>-${totalFees}</p>
            </div>
            ...
            >
              <div>
                <p style={{ fontSize: 16, fontWeight: 700, color: "var(--text-primary)", letterSpacing: "-0.3px" }}>Net Loan Amount</p>
                <p style={{ fontSize: 12, fontWeight: 500, color: "var(--text-secondary)" }}>Amount you will receive</p>
              </div>
              <p style={{ fontSize: 28, fontWeight: 800, color: "var(--color-primary)", letterSpacing: "-0.5px" }}>
      ...
      >
        ...
        >
          ...
          <p className="leading-relaxed" style={{ fontSize: 13, fontWeight: 500, color: "var(--color-primary-active)" }}>
